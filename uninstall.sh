...
if [ -n $TARGET_DEVICE ]; then
  if [ "${TARGET_DEVICE}" = "x86_64" ]; then
    ...
    rm -rf /usr/lib/x86_64-linux-gnu/libv4lconvert.so.0.0.99999
    rm -rf /usr/lib/x86_64-linux-gnu/libv4l2.so.0.0.99999
  fi
fi
update-alternatives --remove deepstream-plugins /opt/nvidia/deepstream/deepstream-${PREV_DS_VER}/lib/gst-plugins
update-alternatives --remove deepstream-app /opt/nvidia/deepstream/deepstream-${PREV_DS_VER}/bin/deepstream-app
update-alternatives --remove deepstream-audio /opt/nvidia/deepstream/deepstream-SDK_VERSION/bin/deepstream-audio
...
update-alternatives --remove deepstream-mrcnn-app /opt/nvidia/deepstream/deepstream-${PREV_DS_VER}/bin/deepstream-mrcnn-app
...
update-alternatives --remove deepstream-infer-tensor-meta-app /opt/nvidia/deepstream/deepstream-${PREV_DS_VER}/bin/deepstream-infer-tensor-meta-app
